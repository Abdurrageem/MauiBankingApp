...
namespace MauiBankingExercise.ViewModels
{
    ...
    public partial class TransactionViewModel : BaseViewModel
    {
        ...
        public TransactionViewModel(DatabaseService databaseService)
        {
            ...
            // Make sure the command is created properly
            SubmitTransactionCommand = new Command(
                execute: async () => await SubmitTransactionAsync(),
                canExecute: () => CanSubmitTransaction());
            TransactionTypes = new List<string> { "Deposit", "Withdrawal" };
        }

        private bool CanSubmitTransaction()
        {
            return TransactionAmount > 0 && !string.IsNullOrEmpty(SelectedTransactionType) && !IsLoading;
        }
        ...
        public decimal TransactionAmount
        {
            ...
            set
            {
                ...
                (SubmitTransactionCommand as Command)?.ChangeCanExecute();
            }
        }
